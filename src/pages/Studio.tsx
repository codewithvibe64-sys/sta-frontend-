...
export default function Studio() {
  ...
  return (
    <main className="pt-24 md:pt-32 overflow-x-hidden selection:bg-[#e03a2f] selection:text-white">
        ...
        <div className="flex flex-col md:grid md:grid-cols-12 gap-8 md:gap-12">
          <header className="md:col-span-8">
            <motion.h1
              ...
              className="text-[clamp(2.5rem,8vw,6rem)] font-black tracking-tighter leading-[0.9] uppercase mb-8 md:mb-12 break-words text-[#f5f5f5]"
            >
              Building the <br />
              <span className="text-[#e03a2f]">Permanent.</span>
            </motion.h1>
            <p className="text-base md:text-xl leading-relaxed text-[#888888] max-w-2xl font-medium">
          ...
          <aside className="md:col-span-4 flex flex-col justify-end">
            <div className="border-l border-[#e03a2f] pl-6 md:pl-8 py-2">
              ...
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#888888] leading-relaxed">
      ...
      {/* Stats (Bonus credibility) */}
      <section className="px-4 md:px-12 mb-24 md:mb-40" aria-label="Studio statistics">
        <div className="grid grid-cols-1 md:grid-cols-3 border border-[#1c1b1b]">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className={`p-8 md:p-12 border-b md:border-b-0 md:border-r border-[#1c1b1b] ${
                index === stats.length - 1 ? "border-b-0 md:border-r-0" : ""
              }`}
            >
              <p className="text-4xl md:text-6xl font-black text-[#f5f5f5] tracking-tighter">
                {stat.value}
              </p>
              <span className="block text-[10px] font-bold uppercase tracking-[0.4em] text-[#444444] mt-4">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      <section className="px-4 md:px-12 mb-24 md:mb-40" aria-labelledby="studio-team-title">
        <div className="flex items-end justify-between mb-16 md:mb-24">
          <h2 id="studio-team-title" className="text-3xl md:text-5xl font-black tracking-tighter uppercase text-[#f5f5f5]">The Practice.</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1">
          {team.map((member, i) => (
            <motion.div 
              ...
              className="group relative aspect-[3/4] overflow-hidden bg-[#131313]"
            >
              <img 
                src={member.img} 
                alt={member.name} 
              ...
              <div className="absolute bottom-8 left-8">
                <p className="text-xl font-bold tracking-tighter uppercase text-[#f5f5f5]">{member.name}</p>
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#e03a2f]">{member.role}</p>
              </div>
            </motion.div>
          ))}
        </div>
              ...
              <div key={idx} className="flex gap-16 items-center">
                {clients.map((client, i) => (
                  <div key={i} className="flex items-center gap-12 group">
                    <span className="text-xl md:text-3xl font-bold uppercase tracking-tight text-[#888888] group-hover:text-[#f5f5f5] transition-colors">{client}</span>
                    <div className="w-2 h-2 bg-[#1c1b1b] group-hover:bg-[#e03a2f] transition-colors"></div>
                  </div>
                ))}
              </div>
            ))}
        ...
        <div className="bg-[#131313] border border-[#1c1b1b] p-8 md:p-24 relative overflow-hidden">
          ...
          <div className="flex flex-col md:grid md:grid-cols-12 gap-12 md:gap-24 relative z-10">
            ...
            <div className="md:col-span-5">
              ...
              <h2 id="studio-careers-title" className="text-3xl md:text-6xl font-black tracking-tighter uppercase mb-12 leading-[0.9] text-[#f5f5f5]">Join the pursuit of permanence.</h2>
              ...
              <div className="space-y-8 mb-16">
                {careers.map((job) => (
                  ...
                  >
                    <div>
                      <h3 className={`text-2xl font-bold uppercase tracking-tight transition-colors ${selectedJobId === job.id ? 'text-[#e03a2f]' : 'text-[#f5f5f5] group-hover:text-[#e03a2f]'}`}>{job.title}</h3>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[#444444]">{job.location}</p>
                    </div>
                    <div className={`h-px transition-all duration-500 ${selectedJobId === job.id ? 'w-20 bg-[#e03a2f]' : 'w-12 bg-[#1c1b1b] group-hover:bg-[#e03a2f] group-hover:w-20'}`}></div>
                  </div>
                ))}
              </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="space-y-8">
                    <div>
                      <h3 className="text-4xl font-black uppercase tracking-tighter text-[#f5f5f5] mb-2">{selectedJob.title}</h3>
                      <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#e03a2f]">{selectedJob.location}</p>
                    </div>
                    <p className="text-[#888888] leading-relaxed">{selectedJob.description}</p>
                    
                    <div className="grid grid-cols-2 gap-12">
                      <div>
                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#f5f5f5] mb-4">Responsibilities</h4>
                        <ul className="space-y-2">
                          {selectedJob.responsibilities.map((item, i) => (
                            <li key={i} className="text-xs text-[#888888] flex gap-3">
                              <span className="text-[#e03a2f]">•</span>
                              {item}
                            </li>
                          ))}
                        </ul>
                      ...
                      <div>
                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#f5f5f5] mb-4">Requirements</h4>
                        <ul className="space-y-2">
                  ...
                  <div className="pt-12 border-t border-[#1c1b1b]">
                    <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#f5f5f5] mb-8">Apply for this position</h4>
                    <div className="w-full bg-[#0f0f0f] border border-[#1c1b1b] overflow-hidden">
                      <iframe 
      ...
      <AnimatePresence>
        {isMobileOverlayOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-[#0f0f0f] overflow-y-auto p-8 md:hidden"
          >
            <div className="flex justify-between items-center mb-12">
              <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#e03a2f]">Job Details</span>
              <button onClick={() => setIsMobileOverlayOpen(false)} className="text-[#f5f5f5] hover:text-[#e03a2f]">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-12 pb-20">
              <div className="space-y-6">
                <h3 className="text-4xl font-black uppercase tracking-tighter text-[#f5f5f5] leading-none">{selectedJob.title}</h3>
                <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#e03a2f]">{selectedJob.location}</p>
                <p className="text-[#888888] leading-relaxed">{selectedJob.description}</p>
              </div>
              ...
              <div className="pt-12 border-t border-[#1c1b1b]">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-[#f5f5f5] mb-8">Apply Now</h4>
                <div className="w-full bg-[#0f0f0f] border border-[#1c1b1b] overflow-hidden">
                  <iframe 
                    src="https://docs.google.com/forms/d/e/1FAIpQLSdw_Xf-X_X_X_X_X_X_X_X_X_X_X_X_X_X_X_X_X_X_X_X/viewform?embedded=true" 
                    width="100%" 
      ...
      >
        <div className="mb-16 md:mb-24 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div>
            <span className="block font-bold uppercase tracking-[0.4em] text-[10px] text-[#444444] mb-4">
              Network
            </span>
            <h2
              id="studio-collab-title"
              className="text-3xl md:text-5xl font-black uppercase tracking-tighter text-[#f5f5f5]"
            >
              Collaborations.
            </h2>
          </div>
          <div className="hidden md:block w-1/3 h-px bg-[#1c1b1b]" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {partners.map((collab) => (
            <article
              key={collab.name}
              className="border-l border-[#1c1b1b] pl-6 md:pl-8 py-4 hover:border-[#e03a2f] transition-colors group"
            >
              <span className="block text-[10px] font-bold uppercase tracking-[0.3em] text-[#444444] mb-2 group-hover:text-[#e03a2f] transition-colors">
                {collab.name}
              </span>
              <h3 className="text-base md:text-lg font-bold uppercase tracking-tight text-[#f5f5f5]">
                {collab.partner}
              </h3>
            </article>
          ))}
